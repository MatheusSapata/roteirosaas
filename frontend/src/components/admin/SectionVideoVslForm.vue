...
const applySelectedPage=()=>{const page=agencyPages.value.find(item=>item.id===Number(local.ctaPageId));const agency=agencyStore.agencies.find(item=>item.id===agencyStore.currentAgencyId);if(page&&agency)local.ctaLink=buildPublicPagePath(page.slug,agency.slug,agencyStore.currentPrimaryDomain)};
const setDestinationMode=(mode:"external"|"page")=>{local.ctaDestinationMode=mode;if(mode==="page")applySelectedPage()};
const loadPages=async()=>{if(!agencyStore.currentAgencyId)return;pagesLoading.value=true;try{agencyPages.value=(await api.get<AgencyPage[]>("/pages",{params:{agency_id:agencyStore.currentAgencyId}})).data;applySelectedPage()}catch{agencyPages.value=[]}finally{pagesLoading.value=false}};
