<template>
  <section class="internal-form-section" :style="sectionStyle">
    ...
    <div class="content" :class="`align-${section.alignment || 'center'}`">
      ...
      <header v-if="title || subtitleHtml"><h2 v-if="title">{{ title }}</h2><div v-if="subtitleHtml" class="rich-subtitle" v-html="subtitleHtml"></div></header>
...
const buttonStyle = computed(() => ({ backgroundColor: form.value?.buttonColor || "#22c55e", color: "#0f172a" }));
...
.content{display:grid;align-items:center;gap:clamp(32px,6vw,80px)}.section-heading{grid-column:1/-1;grid-row:1;width:100%;text-align:center;justify-self:center;margin-bottom:calc(clamp(32px,6vw,80px) * -0.55)}.align-center{grid-template-columns:1fr;justify-items:center;text-align:center}.align-center header{grid-row:2}.align-center .form-card{grid-row:3}.align-left,.align-right{grid-template-columns:minmax(0,520px) minmax(0,1fr)}.align-left .form-card{grid-column:1;grid-row:2}.align-left header{grid-column:2;grid-row:2;text-align:left;margin-bottom:0}.align-right header{grid-column:1;grid-row:2;text-align:left;margin-bottom:0}.align-right .form-card{grid-column:2;grid-row:2}@media(max-width:760px){.content,.align-left,.align-right{grid-template-columns:1fr;justify-items:center;text-align:center}.section-heading{grid-column:1;grid-row:1}.align-left header,.align-right header,.align-center header{grid-column:1;grid-row:2;text-align:center}.align-left .form-card,.align-right .form-card,.align-center .form-card{grid-column:1;grid-row:3}}
